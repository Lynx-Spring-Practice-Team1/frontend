...
export default function OrderPanel({ isDark = false, activeTicker = 'AAPL', currentPrice = 0 }) {
    ...
    return (
        <div className={`rounded-xl border p-4 flex flex-col gap-3 ${isDark ? 'bg-[#1a1a1a] border-[#333]' : 'bg-[#e8e8e8] border-[#d8d8d8]'}`}>
            ...
            {/* Review button */}
            <button
                className="w-full py-2.5 rounded-lg text-sm font-semibold text-white cursor-pointer border-none hover:opacity-90 transition-opacity"
                style={{ backgroundColor: ACCENT }}
            >
                Review {side.charAt(0) + side.slice(1).toLowerCase()} →
            </button>
        </div>
    );
}
